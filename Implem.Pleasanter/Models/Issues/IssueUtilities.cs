...
namespace Implem.Pleasanter.Models
{
    public static class IssueUtilities
    {
        ...
        public static ContentResultInheritance CreateByApi(Context context, SiteSettings ss)
        {
            ...
            if(HasInvalidValueAsApiDataAtCreate(issueApiModel))
            {
                context.InvalidJsonData = !context.RequestDataString.IsNullOrEmpty();
            }
            var issueModel = new IssueModel(
            ...
            var processes = issueApiModel?.ProcessIds != null
                ? ss.Processes?.Where(process => issueApiModel.ProcessIds.Contains(process.Id)).ToList()
                : ss.Processes?.Where(process => process.Id == issueApiModel?.ProcessId).ToList();
        ...
        private static bool HasInvalidValueAsApiDataAtCreate(IssueApiModel model)
        {
            if (model is null)
                return false;
        ...
        public static string BulkProcess(Context context, SiteSettings ss)
        {
                ...
                where: where))
            {
                ...
                else
                {
                    ...
                    {
                        p.MatchConditions = issueModel.GetProcessMatchConditions(
                            context: context,
                            ss: ss,
                            process: p);
            ...
            string previousTitle)
        {
            ...
            if(HasInvalidValueAsApiDataAtUpdate(issueApiModel))
            {
                context.InvalidJsonData = !context.RequestDataString.IsNullOrEmpty();
            }
            var issueModel = new IssueModel(
        ...
        private static bool HasInvalidValueAsApiDataAtUpdate(IssueApiModel model)
        {
            if (model is null)
                return false;
            foreach (var o in model.AttachmentsHash)
            ...
            Column aggregationTarget)
        {
            if (groupBy != null)
            {
                var column = Rds.IssuesColumn()
                    .IssuesColumn(
                        columnName: Rds.IdColumn(tableName: "Issues"),
                        _as: "Id")
                            ...
                            where: new Rds.IssuesWhereCollection()
                                ...
                                .Ver(
                                    sub: Rds.SelectIssues(
                                        tableType: Sqls.TableTypes.History,
                                        _as: "b",
                                        column: Rds.IssuesColumn().Ver(
                                            tableName: "b",
                                            function: Sqls.Functions.Max),
                                        where: Rds.IssuesWhere().IssueId(
                                            tableName: "b",
                                            raw: "\"Issues\".\"IssueId\""),
